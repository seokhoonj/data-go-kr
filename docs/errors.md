...
- **빈 결과는 에러가 아님.** 데이터 없음(reason 코드 03)은 예외가 아니라 **빈 리스트
  `[]`**(CLI는 `(no rows)`)로 옵니다. 그래서 "정상 조회했지만 해당 조건에 데이터가 없음"과
  "지역코드·월·HS/측정소 코드를 잘못 넣어 빗나감"이 똑같이 `[]`입니다. 빈 결과가 나오면
  코드(리졸버로 재확인)·날짜 형식과 범위·오퍼레이션 선택을 점검하고, 필요하면 조회 구간을
  넓혀 보세요.

...
예보구역 named by a ``region`` code (``11B00000`` 서울/인천/경기 for land, ``11B10101`` 서울
...
..``_10``). Which days a call returns depends on ``base_time`` -- the 0600 announcement
...
class MidForecast:
    ...
    ``DATAGOKR_API_KEY`` / the config file)::
        ...
        rows = mid.temperature(region="11B10101", base_time="202608111800") # 기온(최저·최고)
    """
    ...
    def fetch(self, name: str, *, region: str, base_time: str,
        ...
        ``base_time``. ``clean=True`` (the default) returns typed rows; ``clean=False`` raw."""
        try:
            table = TABLES[name]
        except KeyError:

...
def test_land_types_precip_int_and_sky_text():
    ...
    row = mid.land(region_code="11B00000", base_time="202608111800")[0]
    assert row["region_code"] == "11B00000"
...
def test_temperature_types_min_and_max():
    ...
    row = mid.temperature(region_code="11B10101", base_time="202608111800")[0]
...
def test_land_raw_passthrough_keeps_vendor_tokens():
    mid, _ = _mid(_xml([_LAND_ROW], 1))
    assert mid.land(region_code="11B00000", base_time="202608111800", clean=False) == [_LAND_ROW]
...
def test_0600_announcement_keeps_day_four_that_1800_omits():
    ...
    got_0600 = mid.land(region_code="11B00000", base_time="202608110600")[0]
    ...
    got_1800 = mid.land(region_code="11B00000", base_time="202608111800")[0]
...
def test_operation_path_and_params_reach_the_vendor():
    ...
    mid.land(region_code="11B00000", base_time="202608111800")
...
def test_temperature_operation_path_and_params_reach_the_vendor():
    mid, opener = _mid(_xml([], 0))
    mid.temperature(region_code="11B10101", base_time="202608111800")
    query = opener.requests[0].full_url
...
def test_land_with_no_rows_returns_empty_list():
    mid, _ = _mid(_xml([], 0))
    assert mid.land(region_code="11B00000", base_time="202608111800") == []


def test_fetch_rejects_an_unknown_operation():
    mid, _ = _mid(_xml([], 0))
    with pytest.raises(ValueError, match="unknown operation"):
        mid.fetch("sea", region_code="11B00000", base_time="202608111800")
